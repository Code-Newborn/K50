...
{
    uchar verifyflag =0;
    FloatDatas datas_float = FloatDatas();   // 实例化数据结构

    if(dataframe.size() == dataFrameFormat.framebytes)  // 是否为完整数据帧
    {
        for (int i = dataFrameFormat.frameHead_index+1; i < dataFrameFormat.verifyFlag_index; i++)
        {
        ...
        {
            int dataStart = 0;
            dataStart = sizeof(dataFrameFormat.frameHead);
            ...
            // char[]转float，QbyteArray一定要写上数据长度，否则若出现‘/0’会截断数据
            memcpy(&datas_float.float_data1_1, QByteArray(dataFrameFormat.data1,4), 4); // 编码器1角度
            memcpy(&datas_float.float_data1_4, QByteArray(dataFrameFormat.data2,4), 4); // 给定角度1
            memcpy(&datas_float.float_data2_1, QByteArray(dataFrameFormat.data3,4), 4); // 编码器2角度
            memcpy(&datas_float.float_data2_4, QByteArray(dataFrameFormat.data4,4), 4); // 给定角度2

#endif
            // 解析浮点数据调试显示 4位小数
            qDebug()<<QString("%1").arg(datas_float.float_data1_1, 0, 'f', 4)   //
                   << QString("%1").arg(datas_float.float_data1_4, 0, 'f', 4)   //
                   << QString("%1").arg(datas_float.float_data2_1, 0, 'f', 4)   //
                   << QString("%1").arg(datas_float.float_data2_4, 0, 'f', 4);  //

            // 解析浮点数据调试显示
            qDebug()<<datas_float.float_data1_1<<datas_float.float_data1_4<<
                      datas_float.float_data2_1<<datas_float.float_data2_4;
...
void Com::openPort(QString portname)
{
    ...
    if(m_serialPort->open(QIODevice::ReadWrite))    // 用ReadWrite 的模式尝试打开串口
    {
        ...
        qDebug()<<m_serialPort->portName()<<"打开!";
    }
    else
    {
        qDebug()<<"串口打开失败";
...
void Com::closePort(QString portname)
{
    if(m_serialPort->portName()==portname)
    {
        if(m_serialPort->isOpen())
        {
            m_serialPort->clear();
            m_serialPort->close();
            qDebug()<<m_serialPort->portName()<<"关闭!";
        }
    }
}
...
{
    QByteArray datastream = m_serialPort->readAll();      // 字节流接收，长度不定

    if(buffCircle.datalength<buffCircle.Maxbuff)    // 缓冲区未满，可以往里写字节数据
    {
        for(int i=0;i<datastream.size();i++)
        {
            buffCircle.buffdata[(buffCircle.w_Ptr)]=datastream[i];
    ...
    {
        // 验证头、尾，读取数据
        if((buffCircle.buffdata[(buffCircle.r_Ptr)%buffCircle.Maxbuff]==dataFrameFormat.frameHead)&&
                (buffCircle.buffdata[(buffCircle.r_Ptr+dataFrameFormat.framebytes-1)%buffCircle.Maxbuff]==dataFrameFormat.frameTail))
        {
            // **********获取数据************
            QByteArray showdata;
            for(int i=0;i<dataFrameFormat.framebytes;i++)
            {
                showdata+= buffCircle.buffdata[(buffCircle.r_Ptr)%buffCircle.Maxbuff];
                buffCircle.r_Ptr=(buffCircle.r_Ptr+1)%buffCircle.Maxbuff;
                buffCircle.datalength--;
            }
            FloatDatas temp =  unpackData(showdata);
            recDatalList.append(temp);
