    ...
    , ui(new Ui::MainWindow)
{
    ...
    refreshTimer = new QTimer();                // 实例化刷新定时器
    sendTimer = new QTimer();                   // 实例化发送定时器
    scanPort();                                 // 检索串口

    // ***************主界面 信号和槽的连接***************
    connect(this,&MainWindow::openSerial,readwriteCom,&Com::openPort);      // 主线程发送“打开串口”，子线程接收“打开串口”
    connect(this,&MainWindow::closeSerial,readwriteCom,&Com::closePort);    // 主线程发送“关闭串口”，子线程接收“关闭串口”
    connect(readwriteCom,&Com::sendfloats,this,&MainWindow::recvfloats);    // 子线程发送“发送数据”，主线程接收“接收数据”
    ...
    connect(this,SIGNAL(sendFrameData(QByteArray)),readwriteCom,SLOT(sendbytedata(QByteArray)));
}
...
void MainWindow::sendFrame()
{
    ...
    {
        frameToSendFormat.ctrlMode = ui->comboBox_mode->currentIndex();

        QString strdata1 = "156.45";
        QString strdata2 = "60.56";
        QString strdata3 = "5.1";
        QString strdata4 = "3.2";
        float data1 = strdata1.toFloat();
        float data2 = strdata2.toFloat();
        float data3 = strdata3.toFloat();
        float data4 = strdata4.toFloat();

        memcpy(frameToSendFormat.data1.data(), &data1, sizeof(data1));   // float转4字节QByteArray
        memcpy(frameToSendFormat.data2.data(), &data2, sizeof(data2));
        memcpy(frameToSendFormat.data3.data(), &data3, sizeof(data3));
        memcpy(frameToSendFormat.data4.data(), &data4, sizeof(data4));
        ...
        sendFrameCount++;
        frameToSendFormat.FrameCount = (frameToSendFormat.FrameCount+1)%255;    // 帧计数(可以不用取余，+1默认循环);
...
void MainWindow::plotUpdate()
{
    while(dataPtr>showPtr)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            ui->lineEdit_chart1_2->setText(QString::number(showFloatsData.float_data1_6,'f',3));
            break;
        default:
            break;
        }
        ...
        ui->chartplot1->graph(2)->addData(sec,showFloatsData.float_data1_3);    // 【曲线2】陀螺1角速度数据
        ui->chartplot1->graph(3)->addData(sec,showFloatsData.float_data1_4);    // 【曲线3】编码器1角度输入数据
        ui->chartplot1->graph(4)->addData(sec,showFloatsData.float_data1_5);    // 【曲线4】编码器1角速度输入数据
        ui->chartplot1->graph(5)->addData(sec,showFloatsData.float_data1_6);    // 【曲线5】陀螺1角速度输入数据
        ...
        case 1:
            ...
            break;
        case 2:
            ui->lineEdit_chart2_2->setText(QString::number(showFloatsData.float_data2_6,'f',3));
            break;
        default:
            break;
        }
        ...
        ui->chartplot2->graph(2)->addData(sec,showFloatsData.float_data2_3);    // 【曲线2】陀螺2角速度数据
        ui->chartplot2->graph(3)->addData(sec,showFloatsData.float_data2_4);    // 【曲线3】编码器2角度输入数据
        ui->chartplot2->graph(4)->addData(sec,showFloatsData.float_data2_5);    // 【曲线4】编码器2角速度输入数据
        ui->chartplot2->graph(5)->addData(sec,showFloatsData.float_data2_6);    // 【曲线5】陀螺2角速度输入数据
...
void MainWindow::on_pushButton_openPort_clicked()
{
    ...
    if(ui->pushButton_openPort->text()==tr("打开串口"))
    {
        ...
        {
            emit openSerial(ui->comboBox_portNames->currentText());
            datalist.clear();
            dataPtr = datalist.size();                      // 初始化数据指针
            showPtr = datalist.size();                      // 显示指针
...
void MainWindow::on_pushButton_saveData_clicked()
{
    ...
    else
    {
        for (int i = 0; i < datalist.size(); i++)
        {
            ...
            QString float_data1_Float = QString::number(datalist[i].float_data1_1, 'f',4);
            QString float_data2_Float = QString::number(datalist[i].float_data1_4, 'f',4);
            QString float_data3_Float = QString::number(datalist[i].float_data2_1, 'f',4);
            QString float_data4_Float = QString::number(datalist[i].float_data2_4, 'f',4);
...
void MainWindow::on_comboBox_chart1_1_currentIndexChanged(int index)
{
    ...
    {
         ui->chartplot1->graph(plotindex)->setVisible(false);
    }
    ui->chartplot1->graph(index)->setVisible(true);
    ui->chartplot1->replot(QCustomPlot::rpQueuedReplot);    // 图表重绘
...
void MainWindow::on_checkBox_EnableSend_stateChanged(int state)
{
    if(state == Qt::Checked && readwriteCom->m_serialPort->isOpen())
    {
        sendTimer->start(1000/dataSendFreq);   // 以定义发送频率发送
    }
    else
    {
        sendTimer->stop();
